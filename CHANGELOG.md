...
- Deployment artefacts: `Dockerfile` (multi-stage build with non-root UID/GID 10001, OCI labels, HEALTHCHECK), `src/healthcheck.ts` (standalone Docker health probe compiling to `dist/healthcheck.js`, honours `METRICS_PORT` and `METRICS_TLS_CERT_PATH`), `deploy/docker-compose.yml` (read-only, hardened `cap_drop`), `deploy/systemd/technitium-metrics-exporter.service` (DynamicUser, ProtectSystem=strict, systemd hardening set), `deploy/kubernetes/` manifests (single-replica Deployment with runAsNonRoot/readOnlyRootFilesystem/seccomp RuntimeDefault, liveness and readiness probes, NetworkPolicy restricting egress and ingress, optional Prometheus Operator ServiceMonitor example)
- Installation guides (`docs/INSTALL_STANDALONE.md`, `docs/INSTALL_DOCKER.md`, `docs/INSTALL_KUBERNETES.md`): per-deployment-method setup instructions with curl-based verification and readiness-health checks; INSTALL_KUBERNETES.md includes plain scrape_configs snippet and optional ServiceMonitor apply
- Test support for container validation: `test/support/mock-technitium-server.ts` (real HTTP server for testing the exporter as a separate process), `test/integration/deploy/docker-image-hygiene.test.ts` and `docker-container.test.ts` (Docker-gated integration tests with `dockerAvailable()` skip gate, proving image layer hygiene and reaching `/readyz` 200 against mock server)

- Dashboard generator (`scripts/generate-dashboard.ts`): produces `dashboards/technitium-dns.json`, a generated (not hand-edited) Grafana dashboard with the exporter-health row first, per-node overview, zone health with SOA serial divergence as the headline panel, transfer health, protocol split, and cluster state; a drift test proves the committed file matches the generator's current output
- Alert rules (`alerts/technitium-dns.yaml`): all fifteen D§7 rules — `TechnitiumExporterAbsent`, `TechnitiumNodeDown`, `TechnitiumCollectorFailing`, `TechnitiumZoneSerialDivergence`, `TechnitiumZoneTransferStale`, `TechnitiumZoneSyncFailed`, `TechnitiumZoneNotifyFailed`, `TechnitiumZoneExpired`, `TechnitiumZoneDisabled`, `TechnitiumZoneVisibilityMismatch`, `TechnitiumClusterNodeUnreachable`, `TechnitiumClusterNodeStateUnknown`, `TechnitiumServerVersionUnsupported`, `TechnitiumStaleData`, `TechnitiumCardinalityGrowth` — with `promtool` unit tests (`alerts/technitium-dns.test.yaml`) asserting every rule both fires and does not fire, including a dedicated assertion that the `for:` debounce actually gates firing rather than just the underlying condition
- Metric-name cross-check test (`test/unit/scripts/metric-name-cross-check.test.ts`): drives a fully-wired real `TargetRegistry` and the global registry through one poll cycle and asserts every `technitium_`-prefixed name referenced by the alert rules, alert tests, or dashboard actually exists in the rendered output, catching a rename that updates the source but not these artefacts
